...
class SQLiteStore:
    ...
    def _init_tables(self):
        self.conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS candidate_memories (
                ...
                extraction_reason TEXT DEFAULT '',
                language TEXT DEFAULT 'zh-CN',
                conflicts_with TEXT DEFAULT '[]',
                status TEXT DEFAULT 'pending',
                created_at REAL NOT NULL,
                reviewed_at REAL,
                accepted_context_id TEXT DEFAULT '',
                meta TEXT DEFAULT '{}'
            );

            CREATE INDEX IF NOT EXISTS idx_candidates_status ON candidate_memories(status);
            CREATE INDEX IF NOT EXISTS idx_candidates_type ON candidate_memories(memory_type);
            CREATE INDEX IF NOT EXISTS idx_candidates_source_session ON candidate_memories(source_session);
            CREATE INDEX IF NOT EXISTS idx_candidates_created ON candidate_memories(created_at);
        """)
        self.conn.commit()
    ...
    def put(self, ctx: Context) -> None:
        ...
        self.conn.execute("""
            ...
            (id, uri, parent_uri, abstract, overview, content,
             ...
             created_at, updated_at, last_accessed, event_time, valid_from, valid_to,
        ...
        """, (
            ...
            ctx.event_time, ctx.valid_from, ctx.valid_to,
            ctx.access_count, ctx.link_count,
    ...
    def _row_to_context(self, row: sqlite3.Row) -> Context:
        return Context(
            ...
            event_time=row["event_time"], valid_from=row["valid_from"], valid_to=row["valid_to"], access_count=row["access_count"],
